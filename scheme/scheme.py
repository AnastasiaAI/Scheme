...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        expression = scheme_eval(first, env)
        arguments = rest.map(lambda func: scheme_eval(func, env))
        return scheme_apply(expression, arguments, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    if expressions.second is nil:
        ...
        return scheme_eval(expressions.first, env, True) # last call --> tail recursive
    # END PROBLEM 8
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            return self.bindings[symbol] # look up in current frame
        if self.parent: #next, if there is a parent --> look up in its frame
    ...
    def rebind(self, symbol, value):
        ...
        search = self
        while search:
            if symbol in search.bindings:
                search.bindings[symbol] = value
                return
            search = search.parent
#        if symbol in self.bindings:
#            self.bindings[symbol] = value
#        if self.parent:
#            self.parent.bindings[symbol] = value
        # END PROBLEM 3
        raise SchemeError('unknown identifier: {0}'.format(symbol))
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals: # while formals != nil   #for i in range(len(vals)):
...
class Procedure:
    ...
    def eval_procedure(self, operands, env):
        eval_operands = operands.map(lambda operand: scheme_eval(operand, env))
        return scheme_apply(self, eval_operands, env)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Invalid function call')
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_procedure(self, operands, env):
        return scheme_eval(self.apply_macro(operands, env),env)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(expressions.first, Pair) and scheme_symbolp(expressions.first.first):
        procedure = MacroProcedure(expressions.first.second, expressions.second, env)
        env.define(expressions.first.first, procedure)
...
def do_mu_form(expressions, env):
    ...
    return MuProcedure(formals, rest)
    # END PROBLEM 16
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            if scheme_symbolp(expr):  # if it's a primitive --> not list
                return env.lookup(expr)
            elif self_evaluating(expr): # # if atom/strin/None --> not list
                return expr
            # All non-atomic expressions are lists (combinations)
            if not scheme_listp(expr):
                raise SchemeError('malformed list: {0}'.format(repl_str(expr)))
            ...
            if scheme_symbolp(first) and first in SPECIAL_FORMS:
                ...
                # if it is a call expr
            else:
                expression = scheme_eval(first, env)
                # arguments = rest.map(lambda func: scheme_eval(func, env))
                # if isinstance(expression,MacroProcedure):
                #     check_procedure(expression)
                #     return scheme_apply(expression,expression.apply_macro(rest,env),env)
                # else:
                # arguments = rest.map(lambda func: scheme_eval(func, env))

                check_procedure(expression)
                # result = scheme_apply(expression, arguments, env)
                result = expression.eval_procedure(rest,env)
        return result
        # END
