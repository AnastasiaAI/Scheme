...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 2
            src.remove_front() # Removes the dot
            after_dot = scheme_read(src)
            if read_tail(src) is not nil:  # same as:  if src.current() ! = ')':
                raise SyntaxError("A dotted pair must have exactly one item after the dot")
            return after_dot
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
